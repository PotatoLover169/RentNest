from datetime import date
from decimal import Decimal

from django.core.exceptions import ValidationError
from django.db import IntegrityError
from django.test import TestCase

from apps.accounts.models import User
from apps.properties.models import (
    Property,
    PropertyType,
    Unit,
    UnitType,
)

from apps.tenancies.models import (
    Tenancy,
    TenancyStatus,
)


class TenancyModelTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.manager = User.objects.create_user(
            email="manager@example.com",
            password="StrongPassword123!",
            role="PROPERTY_MANAGER",
        )

        cls.tenant = User.objects.create_user(
            email="tenant@example.com",
            password="StrongPassword123!",
            role="TENANT",
        )

        cls.property = Property.objects.create(
            manager=cls.manager,
            name="Sunrise Apartments",
            property_type=PropertyType.APARTMENT,
            description="A residential apartment property.",
            address_line="123 Main Street",
            city="Cebu City",
            province="Cebu",
            postal_code="6000",
        )

        cls.unit = Unit.objects.create(
            property=cls.property,
            unit_number="101",
            unit_type=UnitType.ONE_BEDROOM,
            bedrooms=1,
            bathrooms=Decimal("1.0"),
            monthly_rent=Decimal("15000.00"),
        )

    def test_tenancy_can_be_created(self):
        tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
            security_deposit=Decimal("15000.00"),
        )

        self.assertEqual(tenancy.tenant, self.tenant)
        self.assertEqual(tenancy.unit, self.unit)
        self.assertEqual(tenancy.monthly_rent, Decimal("15000.00"))
        self.assertEqual(
            tenancy.security_deposit,
            Decimal("15000.00"),
        )

    def test_default_status_is_pending(self):
        tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
        )

        self.assertEqual(
            tenancy.status,
            TenancyStatus.PENDING,
        )

    def test_end_date_can_be_null(self):
        tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
        )

        self.assertIsNone(tenancy.end_date)

    def test_negative_monthly_rent_is_invalid(self):
        tenancy = Tenancy(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("-1.00"),
        )

        with self.assertRaises(ValidationError):
            tenancy.full_clean()

    def test_negative_security_deposit_is_invalid(self):
        tenancy = Tenancy(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
            security_deposit=Decimal("-1.00"),
        )

        with self.assertRaises(ValidationError):
            tenancy.full_clean()

    def test_end_date_before_start_date_is_invalid(self):
        tenancy = Tenancy(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 15),
            end_date=date(2026, 8, 14),
            monthly_rent=Decimal("15000.00"),
        )

        with self.assertRaises(ValidationError):
            tenancy.full_clean()

    def test_historical_ended_tenancies_are_allowed(self):
        first_tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2025, 1, 1),
            end_date=date(2025, 12, 31),
            monthly_rent=Decimal("14000.00"),
            status=TenancyStatus.ENDED,
        )

        second_tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 1, 1),
            end_date=date(2026, 7, 31),
            monthly_rent=Decimal("15000.00"),
            status=TenancyStatus.ENDED,
        )

        self.assertNotEqual(
            first_tenancy.pk,
            second_tenancy.pk,
        )

        self.assertEqual(
            Tenancy.objects.filter(
                unit=self.unit,
                status=TenancyStatus.ENDED,
            ).count(),
            2,
        )

    def test_active_tenancy_can_exist(self):
        tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
            status=TenancyStatus.ACTIVE,
        )

        self.assertEqual(
            tenancy.status,
            TenancyStatus.ACTIVE,
        )

    def test_tenancy_reaches_property_manager_through_unit(self):
        tenancy = Tenancy.objects.create(
            tenant=self.tenant,
            unit=self.unit,
            start_date=date(2026, 8, 1),
            monthly_rent=Decimal("15000.00"),
        )

        self.assertEqual(
            tenancy.unit.property.manager,
            self.manager,
        )